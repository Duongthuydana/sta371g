...
\begin{frame}[fragile]%{An autoregressive model for beer production}
    \fontsize{8}{8}\selectfont
      ...
      # Create a lag-1 version   
      beerL1 <- lag(beer, k=-1)
      #combine them in a data frame
      beer_all <- cbind(beer=beer,beerL1=beerL1)
      # Frequency: # of data points per year
      plot(beer~beerL1,data=beer_all,xlab = "last month\'s beer production",ylab="this month\'s beer production",pch=19)
      @  
      \lc
\end{frame}
